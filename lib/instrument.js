...
function instrument(ast, no_prs) {
	estraverse.replace(ast, {
		...
		leave: function(node, parent) {
				...
				if (node.callee.type === 'MemberExpression') {
					...
					if (is_pure(node.callee.object)) {
						if (is_pure(node.callee.property)) {
							...
							seq.push(assign_node(tmp_var_name, node.callee.property));
						}
						seq.push(log_call_node(pos_info(node), callee_expr));
						if(!no_prs) {
							seq.push(log_property_read(pos_info(node.callee), callee_expr));
						}
						seq.push(selector(callee_expr, node['arguments']));
					} else {
						tmp_var_name = get_a_tmp();
						seq.push(assign_node(tmp_var_name, node.callee.object));
						if (is_pure(node.callee.property)) {
							...
							seq.push(assign_node(another_tmp_var_name, node.callee.property));
						}
						seq.push(log_call_node(pos_info(node), callee_expr));
						if(!no_prs) {
							seq.push(log_property_read(pos_info(node.callee), callee_expr));
			...
			if (!no_prs && node.type === 'MemberExpression' && !node.lhs && !node.isCallee) {
...
if (require.main === module) {
	...
	if(process.argv[i] === '--no-property-reads') {
		no_prs = true;
		++i;
	}
	
	if(i >= nargs || nargs > i+1) {
		console.error('Usage: node instrument.js [--no-property-reads] path');
		process.exit(-1);
	}

	console.log(exports.instrument_file(process.argv[i], no_prs));
}