...
function instrument(ast) {
	estraverse.replace(ast, {
		...
		leave: function(node, parent) {
			...
			var tmp_var_name, tmp_var_name_1, callee_expr;
				...
				if (node.callee.type === 'MemberExpression') {
					if (is_pure(node.callee.object)) {
						if (is_pure(node.callee.property)) {
							return seq_expr_node([log_call_node(pos_info(node), clone(node.callee)), 
							...
							return seq_expr_node([assign_node(tmp_var_name, node.callee.property),
								...
								call_expr_node(clone(callee_expr), node['arguments'])
							]);
						}
					} else {
						if (is_pure(node.callee.property)) {
							...
							return seq_expr_node([assign_node(tmp_var_name, node.callee.object),
								...
								log_call_node(pos_info(node), callee_expr),
								log_property_read(pos_info(node.callee), callee_expr),
								call_expr_node(callee_expr, node['arguments'])
							]);
						}
					}
				}
...
function log_property_read(pos, node) {
	if (node.computed) {
		return call_expr_node('__pr', [ pos, clone(node.object), clone(node.property) ]);
	} else {
		return call_expr_node('__pr', [ pos, clone(node.object), literal_node(node.property.name) ]);
	}
}
